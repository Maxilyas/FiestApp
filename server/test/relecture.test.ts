...
// salle dire autre chose que ce qui était arrivé. Les sections suivent les
// numéros des constats.
//
// Chaque constat a son test, qui échouait avant sa correction. Trois autres
// gardent ce qu'une correction ne devait pas casser, et passaient donc déjà :
// l'exclusion qui part au miroir d'un seul tenant (8), le crédit qui repart
// quand quelque chose a changé (6), et le détachement commun des téléphones
// (10), qui n'était qu'un regroupement. Ceux qui jouent une soirée ont leur
// propre serveur jetable : l'expérience de l'un fausserait l'autre.
...
test('un invité exclu rend l’expérience et l’Éclat que la soirée lui avait déjà crédités', () =>
  avecBanc(async banc => {
    ...
    try {
      ...
      const sessionId = await jusquAuPodium(host, quiz, [[[chloe, 0], [alice, 1], [bob, 1]]])
      await jusqua(
...
test('un quiz crédité au podium ne se recrédite pas à « Terminer » quand rien n’a changé', () =>
  avecBanc(async banc =>
    enComptantLesCredits(async credits => {
      ...
      const sessionId = await jusquAuPodium(host, quiz, [[[alice, 0], [bob, 1], [dora, 1]]])
      ...
      // La clôture, elle, crédite toujours : c'est le dernier filet, avant
      // tout effacement.
      ...
      assert.equal(credits.get(aliceId), 2, 'la clôture recrédite la soirée')
    }),
  ))
...
test('ce qui change entre le podium et « Terminer » se crédite quand même', () =>
  avecBanc(async banc =>
    enComptantLesCredits(async credits => {
      ...
      const bob = await invite(banc.url, 'Bob', '🐸')
      const alice = await invite(banc.url, 'Alice', '🦊', { cookie: aliceCookie })
      const salle = [await invite(banc.url, 'Dora', '🐙'), await invite(banc.url, 'Eve', '🐝'), await invite(banc.url, 'Fred', '🐢')]
      const aliceId = profilDe(banc, 'alice')
      const moi = async () =>
      ...
      await jusqua(async () => (await moi()) === reponses + XP.podiumQuiz[0], 'Alice créditée première du quiz')
    }),
  ))
...
// passait déjà, et garde ce qu'elles doivent faire toutes les deux. Un écran
// resté sur une page d'avant envoie encore « Nouvelle soirée » : c'est la
// clôture, et une soirée où rien ne s'est joué renvoie ses téléphones à
// l'entrée.
