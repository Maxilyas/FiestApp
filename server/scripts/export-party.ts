...
//
// Avec une adresse, le script demande /s/<espace>/bilan.json au serveur : c'est
// le plus simple, l'adresse est publique. Avec --db, il lit la base distante
// lui-même, pour le jour où le serveur ne répond plus.
...
// Chaque espace et chaque soirée archivée ont leur dossier : l'export de la
// suivante n'écrase pas celui-ci.
const outDir = path.resolve(options.get('out') ?? path.resolve(here, '../../export', slug ?? '', soiree ?? ''))

console.log(
  (url ? `🌐 ${url}` : `🗄️  ${dbUrl}`) + (slug ? ` · espace ${slug}` : '') + (soiree ? ` · soirée ${soiree}` : ''),
)
const target = { slug, archiveId: soiree }
const review = url ? await reviewFromServer(url, target) : await reviewFromDatabase(dbUrl!, dbToken, target)
