 ...
 */
export function TimerBar({ deadline, duration, ticking }: Props) {
  ...
  useEffect(() => {
    if (!ticking || !urgent || seconds <= 0 || lastTick.current === seconds) return
    lastTick.current = seconds
    sound.tick()
  }, [ticking, urgent, seconds])
  ...
  return (
    <div className="timer">
      ...
      <span className={'timer-seconds' + (urgent ? ' urgent' : '')}>{seconds}</span>
    </div>
  )
}
