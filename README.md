...
Elle se crédite **à la fin de chaque quiz**, et une dernière fois quand la soirée est rangée dans l'historique — juste avant que « Nouvelle soirée » n'efface les journaux. Le téléphone reçoit son profil à jour dans la foulée : « +115 points d'expérience », et le niveau affiché monte pendant la fête. La ligne `(profil, soirée)` est remplacée à chaque fois, jamais ajoutée : recalculer dix fois la même soirée ne crédite jamais deux fois. Les **badges**, eux, attendent l'archivage : ils se décernent sur la soirée entière, et un prix donné trop tôt ne se reprend plus.
...
- **Progress** (`core/progress.ts`) — ce qu'une soirée rapporte aux profils qui l'ont jouée, dérivé des journaux par une fonction pure, comme le souvenir et le bilan. La consolidation, elle, se fait dans `space.ts` : à la fin de chaque quiz puis à l'archivage, la clé `(profil, soirée)` la rendant idempotente.
