...
- **`[inscriptions] soirée finie chez « … » : 12 invités ; 12 inscriptions depuis le démarrage du serveur, sous 7 adresses distinctes`**, une ligne à chaque clôture ou essai effacé. Des téléphones en 4G, sur plusieurs opérateurs, doivent donner **plusieurs** adresses. Une salle en 4G sous **une ou deux** adresses, soirée après soirée, veut dire qu'on lit l'adresse d'un proxy : c'est alors à corriger avant une grande soirée. (Une salle entière sur le wifi de la fête donne une adresse, et c'est normal : c'est celle de la box.) Un redémarrage en pleine soirée fait repartir le compte des inscriptions : la ligne le dit en en comptant moins que d'invités.
- **`[inscriptions] réserve de la soirée vide chez « … » pour l'adresse 3fa9c1d2e0 (x-forwarded-for : 2 entrées)`**, une ligne par adresse et par minute au plus. L'adresse n'y paraît jamais, seulement une empreinte salée, qui change à chaque démarrage. Le nombre d'entrées dit combien d'adresses l'en-tête portait : s'il en compte systématiquement plus d'une, même pour un téléphone qui n'a pas de proxy à lui, il y a plus d'un proxy devant le serveur. Une seule entrée ne prouve rien à elle seule, et un client peut envoyer son propre en-tête : c'est la tendance qui compte, pas une ligne.
